...
Then, we explain \textit{Low/No-code development approach} (see section \ref{background:section:lowcode}) and citizen development of the software products. 
...
Next, we compare high and low-fidelity prototyping based on its advantages and disadvantages \cite{article:prototyping:highlowfidelity}.
...
\begin{figure}[htbp]
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
    \includegraphics[width=0.6\textwidth]{high-fidelity-prototyping.png}
    \caption{Prototyping Page of a Cake Company}
    \label{fig:background:high1}   
  \end{subfigure}
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
    \includegraphics[width=0.6\textwidth]{high-fidelity-2.png}
...
The evaluation of prototypes by users provides crucial feedback in iterative approaches for \ac{it} project management, especially agile methodologies \cite{article:prototyping:schwaber}.
...
Next, figure \ref{fig:background:stepsPrototyping} shows a cycle representing the iterative development with prototypes.
...
\section{Low-Code / No-Code Development Approach}
...
It is a technique developers use to help non-developers design and develop software applications using a \textit{\ac{gui}} supported by a \textit{\ac{lcdp}}.
...
These platforms are designed to accelerate the development process and enable companies to build and deploy custom software solutions quickly. 
...
From the example \ref{fig:background:m1m2}, we need to create models for every entity of reality (e.g., An employee and a CEO are both \textit{Person} in the meta-model but are separate entities in models).
...
\textbf{Task goal:} The goal is to offer some movies that the user should watch. \\
\textit{Bad task: } Watch a movie containing the actor `Mr. T' and actress `Ms. K'. \\
\textit{Good Task: } Watch a movie with more than 6.5/10 ratings. \\
In this example, the participants should be free to compare movies based on their criteria. 
...
\textbf{Task goal:} The goal is to find a movie and show times. \\
\textit{Bad Task: } You want to watch a movie Sunday afternoon. Go to the app and tell where you’d click next. \\
\textit{Good Task: } Use the app to find a movie you'd be interested in watching on a Sunday afternoon. \\
...
\textbf{Task goal:} The goal is to change the user's movie preferences. \\
\textit{Bad Task: } You want to change your movie preferences. Go to the website, sign-in, and change your movie preferences. \\
\textit{Good Task: } Change your movie preferences from `action' to `comedy'. \\\\
...
It is beneficial if we use mixed-method services for a task-based usability study.
...
In this section, we discuss the role that UI experimentation plays in the software development process and how designers can ``prototype with real data'' to improve the usability of the \ac{ui}.
UI experimentation is a vital part of the design process for \ac{ui} products. 
...
UI experimentation allows designers to iterate and improve their designs by creating different \textit{Software Variants} and gathering data and feedback from real users.
...
UI experimentation helps product teams test out ideas early in the process with real-world consumers rather than settling on a single solution and executing it in the final phase \cite{misc:CE:miklos}.
...
As per figure \ref{fig:background:abtesting}, \ac{ce} generally uses A/B testing in a primary case of comparing two variants, A and B, which are controlled and test variables respectively in an experiment.
First, the users or the participants of the study are separated into groups and are assigned one of the two variants.
Since we have \texttt{GridView} and the \texttt{ListView}, one group of the users are assigned with List and the other with Grid View (see figure \ref{fig:background:abtesting}).
